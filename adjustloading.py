#-*-coding:utf-8-*-
"""
マップチェンジの際のマップロードを、全プレイヤーが同時に終わるように調節します。
これによってロードが早く終わった人が暇落ちしにくくなることを期待します。

また、途中参加者のロードはゲーム中のラグの原因の大きな要素です。
NOLAG_SLOWLOAD を True にすることで、途中参加の場合のマップロード速度を制限して、ゲーム中のプレイヤーへのラグを抑えることができます。
※マップチェンジの際のロードではロード速度制限はかかりません。

AoS075の元々のバグである、マップサイズに依らずにロード画面のバーが同じサイズで計算されていて、
小さいマップとかだと30%分くらいのバーの位置でロードが終わったり、大きいマップだと100%になってもロードが終わらなかったりするという現象の対策もしてあります。
