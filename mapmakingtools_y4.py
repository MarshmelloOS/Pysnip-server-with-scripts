# -*- coding: utf-8 -*-
"""
the new function of mapmakingtools

you need make "structure" folder in dist,
and "temp" folder in "structure"folder.
...
/cc(copy) and /cx(cut) use memory so that you can copy&paste the structure over mapchange.

/cc or /cx and hit 2points to determine copy area
and /cv and hit 1point to determine paste point


/cs command can make datatxtfile.
you need name of datafile (ex: /cs blue_house) 
and the operation is same as /cc

if name is empty (only you command "/cs"), the structure save to LINK_SERVER_DIR folder's temp folder.

/cp command can read from datafile that made by /cs
you need name of datafile (ex: /cp blue_house) 
and the operation is same as /cv

if name is empty (only you command "/cp"), the structure which saved by only "/cs" is readed .


commands remember method
/cc :means Ctrl C (copy)
/cx :means Ctrl X (cut)
/cv :means Ctrl V (paste)
/cs :means Ctrl S (save as datafile)
/cp :means Ctrl P (print from datafile)
...
/cs (name)で名前を付けて建築物データをstructureフォルダに保存します
/cp (name)でその名前の建築物データを呼び出してペーストします。（この際、指定点は構造物北西地面側頂点を指定してください。）

名称無しで/cs /cpを打った場合、サーバー跨ぎコピー一時保存ファイルが生成されます。
コピー元サーバーで/csで保存を行い、ペースト先サーバーで/cpを打つと構造物がコピーされます。

/cs /cp使用のため、distフォルダ内に"structure"フォルダを、さらにその中に"temp"フォルダーを作ってください
サーバー跨ぎコピーの送り先フォルダは70行目辺りのLINK_SERVER_DIRで指定します
...
LINK_SERVER_DIR = "../../pys-main-32887/dist"   #　　サーバー跨ぎ送り先フォルダのrun.exeのあるフォルダ（送信元サーバーのrun.exeから見た相対参照）
...
def ctrls(*arguments):
	connection = arguments[0]
	connection.reset_build()
	connection.callback = ctrls_r
	connection.arguments = arguments
	connection.select = True
	connection.points = 2
	connection.send_chat('Hit 2 blocks to determine the region to be saved.(ver.y)')
add(ctrls)

def ctrls_r(connection, name="name_not_set"):
	x1 = min(connection.block1_x, connection.block2_x)
	x2 = max(connection.block1_x, connection.block2_x)
	y1 = min(connection.block1_y, connection.block2_y)
	y2 = max(connection.block1_y, connection.block2_y)
	...
	connection.ctrls_mem=[]

	for xx in xrange(0, x2 + 1 - x1):
		connection.ctrls_mem.append([])
		for yy in xrange(0, y2 + 1 - y1):
			connection.ctrls_mem[xx].append([])
			for zz in list(reversed(range(0, z2 + 1 - z1))):
				if connection.protocol.map.get_solid(xx+x1, yy+y1, zz+z1):
					...
					color = connection.protocol.map.get_color(xx+x1, yy+y1, zz+z1)

					connection.ctrls_mem[xx][yy].append(color)
				else:
					connection.ctrls_mem[xx][yy].append(None)

	if name == "name_not_set":
		with open('%s/structure/temp/%s.txt'%(LINK_SERVER_DIR, connection.name), 'w') as file:
			json.dump(connection.ctrls_mem, file, indent = 4)
		connection.send_chat('structure was sent to linked server.')

	else:
		if not os.path.exists('./structure/%s.txt'%name):
			with open('./structure/%s.txt'%name, 'w') as file:
				json.dump(connection.ctrls_mem, file, indent = 4)
				connection.send_chat('structure saved as "%s".txt'%name)
		else:
			connection.send_chat('ERROR! structure name "%s" was already used.'%name)
...
def ctrlp(*arguments):
	connection = arguments[0]
	connection.reset_build()
	connection.callback = ctrlp_r
	connection.arguments = arguments
	connection.select = True
	connection.points = 1
	connection.send_chat('Hit 1 blocks to determine the region to be pasted from saved data.(ver.y)')
add(ctrlp)

def ctrlp_r(connection, name = "name_not_set"):
	...
	if name == "name_not_set":
		if os.path.exists('./structure/temp/%s.txt'%connection.name):
			with open('./structure/temp/%s.txt'%connection.name, 'rb') as file:
				data = json.load(file)
		else:
			errored = True
			connection.send_chat('ERROR! there is No temp file')
	else:
		if os.path.exists('./structure/%s.txt'%name):
			with open('./structure/%s.txt'%name, 'rb') as file:
				data = json.load(file)
		else:
			errored = True
			connection.send_chat('ERROR! filename "%s".txt is not exist.'%name)
