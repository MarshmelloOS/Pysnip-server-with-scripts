"""
no battle for 20 sec on marker script
by yuyasato
...
ALIVE_ONLY= True
EXPLODE_EFFECT = False  # explode sound and effect at the position on the timing of marker.

TIMEMARKER_FREQ = 20 #seconds
NOBATTLE_TIME = 30 #seconds
SPAWN_IS_BATTLE=True
...
def time_marker(connection):
	connection.protocol.timemarker_looping = not connection.protocol.timemarker_looping
	connection.protocol.marker_time_loop()
add(time_marker)

def start_tm(self):
	if not self.timemarker_looping:
		self.timemarker_looping=True
		self.marker_time_loop()
def stop_tm(self):
	self.marker_time_loop_stop()
...
def apply_script(protocol, connection, config):
	class MarkerConnection(connection):
		...
		def on_hit(self, hit_amount, hit_player, type, grenade):
			if self!=hit_player:
				self.protocol.battle_stump = seconds()
				...
				if not self.disconnected:
					for marker in self.protocol.markers:
						if marker.whose == self:
							marker.expire()
					if seconds() > self.protocol.battle_stump + NOBATTLE_TIME:
						if not ALIVE_ONLY or not self.world_object.dead:
	...
	class MarkerProtocol(protocol):
		...
		def __init__(self, *arg, **kw):
			...
			if self.timemarker_looping:
				self.marker_time_loop()

		def marker_time_loop_stop(self):
			self.timemarker_looping=False
			for player in self.blue_team.get_players():
				for marker in self.markers:
					if marker.whose == player:
						marker.expire()
