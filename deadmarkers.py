...
REVEAL_ENEMIES = False # if True, mimics old intel reveal behavior when captured
...
def apply_script(protocol, connection, config):
	class MarkerConnection(connection):
		...
		def make_marker_send(self,Enemy):
			if self:
				if not self.disconnected:
					if not self.hp:
						location = self.dummy_body.position.x, self.dummy_body.position.y,self			
						self.make_marker(Enemy,location)

		def on_kill(self, killer, type, grenade):
			if self.team == self.protocol.blue_team:
				callLater(2.5,self.make_marker_send,Enemy)
