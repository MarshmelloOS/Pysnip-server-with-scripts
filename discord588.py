...
DISCORD_SERVER_URL = "https://discord.gg/waZqYRHW"  # Your Discord server URL
...
def apply_script(protocol, connection, config):
    ...
    class DiscordHelper(connection):
        ...
        def on_user_login(self, user_type, verbose=True):
            ...
            message = '`%s` logged in as `%s` on server `%s`. Join our Discord server: %s' % (
                self.name, user_type, protocol.name, DISCORD_SERVER_URL)
