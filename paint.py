...
mod yuyasato
/fd [number]
color depth change. if /fd 2 -> color will be 2 times dark, /fd 0.33 -> color will be 3 times light
/fc ["rbg","brg","bgr","grb" or "gbr"]
color change. if /fc rbg  -> RGB will be changed RBG  , /fc grb -> RGB willbe GRB
"""
...
@name('filldepth')
@admin
def filldepth(*arguments):
	connection = arguments[0]
	connection.reset_build()
	connection.callback = filldepth_r
	connection.arguments = arguments
	connection.select = True
	connection.points = 2
	connection.send_chat('Paint 2 blocks to determine the region to be color depth changed.')
add(filldepth)

def filldepth_r(connection, depth=1):
		depth = float(depth)
		x1 = min(connection.block1_x,connection.block2_x)
		y1 = min(connection.block1_y,connection.block2_y)
		z1 = min(connection.block1_z,connection.block2_z)
...
@name('fillchange')
@admin
def fillchange(*arguments):
	connection = arguments[0]
	connection.reset_build()
	connection.callback = fillchange_r
	connection.arguments = arguments
	connection.select = True
	connection.points = 2
	connection.send_chat('Paint 2 blocks to determine the region to be color depth changed.')
...
def fillchange_r(connection, pattern="rdm"):
		x1 = min(connection.block1_x,connection.block2_x)
		y1 = min(connection.block1_y,connection.block2_y)
		z1 = min(connection.block1_z,connection.block2_z)
		x2 = max(connection.block1_x,connection.block2_x)
			...
			for y in xrange(y1, y2 + 1):
				for z in xrange(z1, z2 + 1):
					if connection.protocol.map.get_solid(x, y, z):
						...
						g=color[1]
						b=color[2]
						ptns = ["rbg","brg","bgr","grb","gbr"]
						if not pattern in ptns:
							random.choice(ptns)
						if pattern == "rbg":
							color = (int(r), int(b), int(g), 255)
						elif pattern == "brg":
							color = (int(b), int(r), int(g), 255)
						elif pattern == "bgr":
							color = (int(b), int(g), int(r), 255)
						elif pattern == "grb":
							color = (int(g), int(r), int(b), 255)
						elif pattern == "gbr":
							color = (int(g), int(b), int(r), 255)
						set_color(connection.protocol, make_color_tuple(color))
