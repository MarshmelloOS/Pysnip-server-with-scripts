    ...
    command:
        ...
        set <action>
            ...
            action:
                ...
                capture <amount team>
                    Activating players capture flag for amount.
                    Team is expected 'b'(='blue'), 'g'(='green') or 'n'('neutral')
                    A player can't get points from the button which has the same team as player
                restore <team>
                    Activating players are refilled his health and ammo
                    Team is expected 'b'(='blue'), 'g'(='green') or 'n'('neutral')
                    A player can't get refilled by enemy's button
                fog <r> <g> <b>
                    Change fog color.
                    r, g, b can have value in 0 to 255.
        list
    ...
    command:
        ...
        set [not] <action>
            ...
            action:
                ...
                shot
                    Fires when a player normally hits the button with the weapon.
                nade
                    Fires when a grenade explodes around the button.
                distance [radius=3]
...
from pyspades.server import block_action, block_line, set_color, position_data, grenade_packet
...
S_ACTION_DAMAGE_USAGE = 'Usage: /action add damage <amount team>'
S_ACTION_CAPTURE_USAGE = 'Usage: /action add capture <amount team>'
S_ACTION_RESTORE_USAGE = 'Usage: /action add restore <team>'
S_ACTION_FOG_USAGE = 'Usage: /action add fog <r g b>'
...
S_CAPTURED = '[WARNING] {player} captured button in {coords}'
...
ACTION_ADD_ACTIONS = ('height', 'raise', 'lower', 'elevator', 'output',
    'teleport', 'chat', 'damage', 'capture', 'restore', 'fog')
ACTION_ADD_USAGES = {
    ...
    'capture' : S_ACTION_CAPTURE_USAGE,
    'restore' : S_ACTION_RESTORE_USAGE,
    'fog' : S_ACTION_FOG_USAGE,
}
...
TRIGGER_ADD_TRIGGERS = ('press', 'shot', 'nade', 'distance', 'track', 'height')
TRIGGER_ADD_USAGES = {
        ...
        if command in ('add', 'set'):
            ...
            if action in ('height', 'raise', 'lower', 'elevator'):
                ...
                for parameter, value in kwargs.iteritems():
                    ...
                    #    raise ValueError(message)
                    if parameter != 'height' and type(value) in (int, float) and value < 0:
                        message = S_NOT_POSITIVE.format(parameter = parameter)
                        raise ValueError(message)
                    # MOD END
                ...
                team = args[3]
                if team == 'b' or team == 'blue':
                    teamname = 'blue'
                elif team == 'g' or team == 'green':
                    teamname = 'green'
                else:
                    teamname = 'neutral'
                new_state.kwargs = {'value' : amount, 'type' : damage_type, 'team' : teamname}
            elif action == 'capture':
                amount = (int)(args[2])
                team = args[3]
                if team == 'b' or team == 'blue':
                    ...
                    teamname = 'neutral'
                new_state.kwargs = {'value' : amount, 'team' : teamname}
                ...
                if team == 'b' or team == 'blue':
                    ...
                    teamname = 'neutral'
                new_state.kwargs = {'team' : teamname}
            
            elif action == 'fog':
                r = (int)(args[2])
                g = (int)(args[3])
                b = (int)(args[4])
                new_state.kwargs = {'color' : (r, g, b)}
        else:
        ...
        if command in ('add', 'set'):
            ...
            if command == 'del':
                ...
                if new_state.number != 'all':
                    ...
                    # MOD BEGIN [CRASIVE]
                    player.action_ray_length_tmp=ACTION_RAY_LENGTH_EM
                    # MOD END
            elif command == 'logic':
...
TRIGGER_CLASSES = {}
for cls in (PressTrigger, ShotTrigger, NadeTrigger, TrackTrigger, DistanceTrigger, HeightTrigger):
...
class Platform(BaseObject):
    ...
    def contains(self, x, y, z):
        ...
        #     self.start_z)
        if self.z <= self.start_z:
            return aabb(x, y, z, self.x1, self.y1, self.z, self.x2, self.y2, self.start_z)
        else:
            return aabb(x, y, z, self.x1, self.y1, self.start_z, self.x2, self.y2, self.z)
        # MOD END
    ...
    def overlaps(self, p):
        ...
        if self.z <= self.start_z:
            if p.z <= p.start_z:
                ...
                return (self.x1 <= p.x2 and self.y1 <= p.y2 and self.z <= p.z and
                    self.x2 >= p.x1 and self.y2 >= p.y1 and self.start_z >= p.start_z)
        else:
            if p.z <= p.start_z:
                return (self.x1 <= p.x2 and self.y1 <= p.y2 and self.start_z <= p.start_z and
                    self.x2 >= p.x1 and self.y2 >= p.y1 and self.z >= p.z)
            else:
                return (self.x1 <= p.x2 and self.y1 <= p.y2 and self.start_z <= p.z and
                    self.x2 >= p.x1 and self.y2 >= p.y1 and self.z >= p.start_z)
        # MOD END
    ...
    def destroy(self):
        ...
        if self.z <= self.start_z:
            ...
            self.destroy_z(self.start_z, self.z + 1)
        # MOD END
        self.release()
    ...
    def start(self, height, mode, speed, delay, wait = None, force = False):
        ...
        self.target_z = self.target_z = max(0, min(62, self.start_z - height))
        # MOD END
        self.speed = speed
        self.wait = wait
    ...
    def cycle(self):
        ...
        # if self.z > self.target_z:
        #    self.z -= 1
        if ( self.z > self.target_z and self.z <= self.start_z ) or ( self.z < self.target_z and self.z >= self.start_z ):
            if self.z > self.target_z:
                self.z -= 1
            #MOD END
        ...
        #    self.protocol.update_entities()
        #    self.z += 1
        elif ( self.z < self.target_z and self.z < self.start_z ) or ( self.z > self.target_z and self.z >= self.start_z ):
            if self.z > self.target_z:
                self.z -= 1
            self.destroy_z(self.z)
            self.protocol.update_entities()
...
def player_action(player, select, inspect, type):
    ...
    location = player.world_object.cast_ray(player.action_ray_length_tmp)
    if location is None:
        return
    x, y, z = location
    ...
    if button:
        ...
        if not inspect:
            for trigger in button.triggers:
                if trigger.type == 'press' and type == 'press':
                    trigger.callback(player)
                if trigger.type == 'shot' and type == 'shot':
                    trigger.callback(player)
...
class TriggerAddState(State):
    ...
    def on_exit(self, protocol, player):
        ...
        if self.trigger == 'shot':
            new_trigger = ShotTrigger(protocol)
        if self.trigger == 'nade':
            new_trigger = NadeTrigger(protocol)
...
class TriggerCommandState(State):
    ...
    def on_exit(self, protocol, player):
        # MOD BEGIN [CRASIVE]
        if player.action_ray_length_tmp == ACTION_RAY_LENGTH_EM:
            player.action_ray_length_tmp = ACTION_RAY_LENGTH
        # MOD END
        button = self.button
        if not button:
...
def apply_script(protocol, connection, config):
    class PlatformConnection(connection):
        ...
        # MOD BEGIN [CRASIVE]
        action_ray_length_tmp=ACTION_RAY_LENGTH_EM
        # MOD END
        ...
        def on_reset(self):
            ...
            # MOD BEGIN [CRASIVE]
            self.action_ray_length_tmp = ACTION_RAY_LENGTH_EM
            # MOD END
        ...
        def on_block_destroy(self, x, y, z, mode):
            ...
            if mode == DESTROY_BLOCK:
                ...
                destroyable = connection.on_block_destroy(self, x, y, z, mode)
                for i, j, k in prism(x - 1, y - 1, z - 1, x + 2, y + 2, z + 2):
                    if is_platform(i, j, k):
                        destroyable = False
                        self.last_action = seconds()
                        button = self.protocol.buttons.get((i, j, k))
                        if button:
                            for trigger in button.triggers:
                                trigger.callback(self)
                return destroyable
        ...
        def on_shoot_set(self, fire):
            if self.tool == SPADE_TOOL and fire:
                player_action(self, True, False, 'press')
            if self.tool == WEAPON_TOOL and fire:
                player_action(self, True, False, 'shot')
            connection.on_shoot_set(self, fire)
        
        def on_position_update(self):
            if self.tool == SPADE_TOOL:
                player_action(self, self.world_object.primary_fire, False, 'press')
            if self.tool == WEAPON_TOOL:
                player_action(self, self.world_object.primary_fire, False, 'shot')
        ...
        def on_animation_update(self, jump, crouch, sneak, sprint):
            if self.tool == SPADE_TOOL:
                ...
                player_action(self, self.world_object.primary_fire, inspect, 'press')
            if self.tool == WEAPON_TOOL:
                inspect = not self.world_object.sneak and sneak
                player_action(self, self.world_object.primary_fire, inspect, 'shot')
        ...
        def damage_block(self, **args):
            amount = args['value']
            type = args['type']
            teamname = args['team']
            print(amount, type, teamname)
            if teamname == 'blue':
                team = self.protocol.blue_team
            elif teamname == 'green':
                team = self.protocol.green_team
            else:
                team = teamname
            if self.team is not team:
                self.hit(amount)
        
        def capture_block(self, **args):
            amount = args['value']
            teamname = args['team']
            if teamname == 'blue':
                team = self.protocol.blue_team
            ...
            if self.team is team:
                self.send_chat('You cannot capture the bottun belong to your team.')
            else:
                while counter < amount:
                    self.take_flag()
                    self.capture_flag()
                    counter += 1
                self.send_chat('you get %d points' %amount)
                x, y, z = self.world_object.position.get()
                coords = get_coordinate(x, y)
                message = S_CAPTURED.format(player = self.name, coords = coords)
                self.protocol.send_chat(message, global_message = False, team = self.team.other)
        
        def restore_block(self, **args):
            teamname = args['team']
            if teamname == 'blue':
                team = self.protocol.blue_team
            elif teamname == 'green':
                ...
                team = teamname
            if self.team.other is team:
                self.send_chat('You cannot get restored by the button of enemy')
            else:
                self.refill()
                self.send_chat('Your health and ammo is restored')
        
        def dye_fog(self, **args):
            color = args['color']
            self.protocol.set_fog_color(color)
        
        def on_tool_changed(self, tool):
            if tool == WEAPON_TOOL:
                self.action_ray_length_tmp = ACTION_RAY_LENGTH_EM
            else:
                self.action_ray_length_tmp = ACTION_RAY_LENGTH
